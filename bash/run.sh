...
if [ -z "$url" ]; then
    ...
    echo "Example: ./run.sh example.com"
    exit 1
fi
if [ ! -d "$url" ]; then
	mkdir $url
...
echo "[+] Probing for a live domains ..."
...
cat $url/recon/wayback/wayback_output.txt | grep '?*=' | cut -d '=' -f1 | sort -u >> $url/recon/wayback/params/wayback_params.txt 
